...
% * raw        - box dimensions input [nframe x 6 double]
% *              [A, cos(gamma), B, cos(beta), cos(alpha), C]
% * box        - box size [nframe x 9 double]
% *              [ax = A, ay = 0, az = 0, ...
% *               bx = B*cos(gamma), by = sqrt(B^2-bx^2), bz = 0, ...
% *               cx = C*cos(beta), cy = (B*C*cos(alpha)-bx*cx)/by, cz = C^2-cx^2-cy^2];
...
for iframe = 1:nframe
  ...
  if header.is_charmm_extrablock
     ...
     box(iframe,2) = 0.0;
     box(iframe,3) = 0.0;
     box(iframe,4) = raw(3)*raw(2);
     box(iframe,5) = sqrt(raw(3)^2-box(iframe,4)^2);
     box(iframe,6) = 0.0;
     box(iframe,7) = raw(6)*raw(4);
     box(iframe,8) = (raw(3)*raw(6)*raw(5)-box(iframe,4)*box(iframe,7))/box(iframe,5);
     box(iframe,9) = sqrt(raw(6)^2-box(iframe,7)^2-box(iframe,8)^2);
  end
